...
export default function() {
  postman[Pre].push(() => {
    ...
    pm.variables.set(
      "ConnectionName",
      "MSteamsConn" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsSendMessageCUName",
      "MSteamssendMessage" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsOneonOneChatCUName",
      "MSteamsOneonOneChat" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsGroupChatCUName",
      "MSteamsGroupChat" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsCreateTeamCUName",
      "MSteamsCreateTeam" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsCreateChannelCUName",
      "MSteamsCreateChannel" + pm.variables.get("RandomNumber")
    );
    pm.variables.set(
      "TeamsScheduleMeetCUName",
      "MSteamsSchedulemeet" + pm.variables.get("RandomNumber")
    );
    pm.variables.set("CUName", "MSteamssendMessage2022051710201015140");
    pm.variables.set("ConnectionId", 1672776860873);
    pm.variables.set("CUdsdId", 545631015841);
  });

  postman[Request]({
    name: "qa3AdminToken Copy",
    id: "22935f38-9fe3-4184-a82e-e3f60c1b7be6",
    method: "POST",
    address:
  ...
  postman[Request]({
    name: "teams_SendMessage Copy",
    id: "eb2eeb84-d1b5-4537-bf5c-2d8211ac0d86",
    method: "POST",
    address: "https://{{BaseURL}}/dsd-orch/adapter/api/v2/integration",
    data: {
      integration:
        '{\n    "integrationName": {{TeamsSendMessageCUName}},\n    "adapterType": "MSTEAMS",\n    "operation": "CREATE_ONEONONE_CHAT",\n    "configEntityRecordId": {{ConnectionId}},\n    "configEntityRecordName" : {{ConnectionName}}\n}'
    },
    headers: {
    ...
    post(response) {
      pm.test("Check status code", function() {
        ...
        pm.expect(pm.response.json().result).to.eq(
          pm.variables.get("TeamsSendMessageCUName")
        );
      });
    }
  });
  ...
  postman[Request]({
    name: "getCUBydsdId1 Copy",
    id: "80e4dab4-6c9d-4cde-885e-798ce8912767",
    method: "GET",
    address:
      "https://{{BaseURL}}/dsd-orch/adapter/api/v2/integration/{{CUdsdId}}",
    headers: {
      Authorization: "{{BearerToken}}"
    },
    post(response) {
      pm.test("Check status code", function() {
        pm.expect(pm.response.code).to.eq(200);
        pm.expect(pm.response.json().message).to.eq("Success");
        pm.expect(pm.response.json().result.integrationName).to.eq(
          pm.variables.get("CUName")
        );
        pm.expect(pm.response.json().result.inputEntityId).to.not.eq(null);
        pm.expect(pm.response.json().result.outputEntityId).to.not.eq(null);
      });
    }
  });
